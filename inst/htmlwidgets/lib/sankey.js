d3.sankey = function() {
  var sankey = {},
      ...
      links = [],
      sinksRight = true,
      pinToTopCore = false; // <-- pin to top flag
 ...
 sankey.pinToTopCore = function(_) {
    if (!arguments.length) return pinToTopCore;
    pinToTopCore = !!_;
    return sankey;
 };
  ...
  function computeNodeBreadths() {
    ...
    remaining = remainingNodes; // this stuff is useless
  ...
  function computeNodeDepths(iterations) {
    ...
    // for pin nodes to top
    function normalizeBreadthTops() {
      nodesByBreadth.forEach(function(nodes) {
        var minY = d3.min(nodes, function(n){ return n.y; });
        nodes.forEach(function(n){ n.y -= minY; });
      });
    }
